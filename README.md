...
工具调用同样默认折叠成**一行概览**：`✓ ⚙ run_shell  echo hi`（状态 mark + 工具名 +
最有信息量的参数）。点击标题展开后可看到完整参数 JSON 与结果输出（超过 12 行截断并
标注剩余行数）。要点：

- 运行中概览带 `…` 标记，完成后变 `✓` / `✗`（失败整行标红）。
- 没有对应 start 的结果（用户拒绝确认、子代理汇总）会自成一块，直接以完成态显示。
- 概览行常驻可见，输出细节按需展开——长输出不再刷屏。
