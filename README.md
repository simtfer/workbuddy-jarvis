...
- **v0.6.3**：子任务面板。派发的子 Agent 显示为可折叠面板（概览一行 + 每个子任务一行实时状态），点击行进入详情页查看完整输出；默认超时 30s→120s、max_runtime 600s→900s
- **v0.6.4**：TUI 代码拆分。`app.py` 1578 行 → 613 行，斜杠命令、系统面板、样式各自成模块（行为不变）
- **v0.6.5**：侧栏鼠标开关。收起时点 `≡` / `◀` 展开，展开时点侧栏顶部标题收起，hover 有高亮
...
- **两个侧栏默认都是收起的**，聊天区先占满窗口；左侧只留一条 `≡`、右侧一条 `◀` 边栏提示它们的存在。面板收起时连进程采样都停掉，不白烧 CPU。
- **侧栏也可以用鼠标开关**：收起时点左边栏的 `≡` / 右边栏的 `◀` 展开，展开时点左边栏顶部的「菜单」或右边栏顶部的「系统面板」收起——每块只在它该起作用的状态下出现，一次点击不会有两种含义。
...
**子任务面板**（在 TUI 里看得最清楚）：
1. 派发后聊天流出现「**并行子任务 · N 个 · 完成 X/N**」折叠面板（默认收起，紧凑排列）。
2. 展开面板列出每个子任务一行：`✓ #1 research A  1.5s`（状态 mark + 序号 + prompt 预览 + 耗时），
   状态实时刷新（pending `·` / running `▶` / done `✓` / error `✗` / timeout `⏱`）。
3. **点击任意一行进入详情页**：该子任务的完整 prompt、状态、用时、工具次数与**完整输出**，
   Esc 返回聊天。
4. 跑满 `Config.subagents.default_timeout`（默认 120 秒）时输出部分快照提示，主 Agent
   **不会**被这里打断；全部落地后汇总自动作为工具结果喂给主 Agent 做最终综合回答。

**配置**（`config.toml` 的 `[subagents]`）：

| 字段 | 默认 | 含义 |
|---|---|---|
| `default_timeout` | 120.0 | 触发「部分快照」的秒数（仅影响 UX，不取消子 Agent） |
| `max_runtime` | 900.0 | 单个子 Agent 的绝对最长用时，超过会被中止 |
...
快捷键：`Ctrl+Q` 退出 · `Ctrl+L` 清屏 · `Ctrl+B` 命令菜单 · `Ctrl+S` 系统面板 · `F1` 帮助 · `F2` 任务 · `Ctrl+X` 取消任务（两个侧栏默认收起，也可以用鼠标点边栏开关）
