...
- 当用户的请求天然包含**多个互不依赖**的子任务时，使用 `delegate_subagents` 工具一次派多个
  子 Agent 并行执行（每个独立对话、共享工具/数据库）。典型场景：
  * 并行搜索多组关键词、并行抓多个 URL；
  * 同时读 / 总结多份文件；
  * 并行跑多个独立实验、对比 N 个 prompt；
  * 「同时做 A、B、C 三件事」。
  不适合：步骤之间有依赖、必须串行的任务 —— 这种情况按普通流程一步步来。
  派发后框架会用 `Config.subagents.default_timeout`（默认 30s）触发一次「部分结果」展示给用户，
  之后所有子 Agent 完成时再展示最终汇总。派发的 prompt 要写得自包含，让子 Agent 知道做什么。
